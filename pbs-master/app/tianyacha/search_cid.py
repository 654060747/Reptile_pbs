...
def check_one_city(city_name, city_abbr):
	...
	for x_code in code:
		...
		for x_ot in range(1,12):
		# for x_ot in range(2,3):
			url_strff = "https://www.tianyancha.com/search/os1-ot"+str(x_ot)
			nex_url = url_strff + "/p1/" + x_code_str

			try:
				get_page = open_city_list_page_url(nex_url)
				page_size_text = get_page.select('#search > span.tips-num')[0].getText()
				if "+" in page_size_text:
					page_size = 250
				elif int(page_size_text) > 5000:
					page_size = 250
				else:
					page_size = math.ceil(int(page_size_text)/20)
			except Exception as e:
				page_size = 0

			print("========================== 即将爬取新的数据 ==========================")
			print(nex_url)
			print("总页数："+str(page_size))

			# print("请输入总本次数据总页数, page=====")
			# page_size = input("");
			for x_page in range(1,page_size+1):
				page = str(x_page)
