...
def get_csv_load_url(csv_url):
	...
	for x in range(1,len(tr_list)+1):
		...
		if 'xls' in csv_name or 'XLS' in csv_name:
			css = css + ' > td:nth-child(3) > a'
			id_InRc = select_css(3, data, css, 'id')
			file_name = select_css(2, data, css, '')
			# print(id_InRc)
			cata_Id = csv_url.split('catalog/')[1]
			csv_load_url = 'http://data.sd.gov.cn/portal/catalog/download?cataId=' + cata_Id + '&idInRc=' + id_InRc
			# print(csv_load_url)
...
def get_csv_id(url):
	...
	for x in range(1,len(li_list)+1):
		...
		csv_url = 'http://data.sd.gov.cn' + csv_url_
...
for x in range(592,662):

	print('第' + str(x) + '页')

	# url = 'http://data.sd.gov.cn/portal/catalog/index?domainId=domain-9&fileFormat=4&openType=1&page=' + str(x)
	url = 'http://data.sd.gov.cn/portal/catalog/index?domainId=domain-6&fileFormat=1&openType=1&page=' + str(x)
